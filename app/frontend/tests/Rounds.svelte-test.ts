  ...
  describe("as organizer", () => {
    describe("with no stages", () => {
      ...
      it("creates a new swiss stage", async () => {
        ...
        vi.spyOn(MockPairingsData, "stages", "get").mockReturnValue([MockSwissStage]);
        await user.click(screen.getByRole("button", { name: /add swiss stage/i }));
    ...
    describe("with no completed rounds", () => {
      ...
      describe.each([
        ...
        it(testName, async () => {
          ...
          vi.spyOn(MockPairing1, "two_for_one", "get").mockReturnValue(twoForOne);
      ...
      it("deletes a stage", async () => {
        ...
        vi.spyOn(MockPairingsData, "stages", "get").mockReturnValue([]);
        vi.spyOn(window, "confirm").mockReturnValue(true);
        await user.click(getByRole(stageDiv, "button", { name: /delete stage/i }));
    ...
    describe("with one round completed", () => {
      ...
      it("pairs a new round", async () => {
        ...
        vi.spyOn(MockPairingsData.stages[0], "rounds", "get").mockReturnValue([MockRound1, MockRound2]);
  
        await user.click(screen.getByRole("button", { name: /pair new round!/i }));
  ...
  describe("as player", () => {
    ...
    describe("does not show", () => {
      ...
      it("the TO stage controls", () => {
        ...
        expect(stageDiv).not.toContainElement(queryByRole(stageDiv, "link", { name: /edit stage/i }));
        expect(stageDiv).not.toContainElement(queryByRole(stageDiv, "button", { name: /delete stage/i }));
      });
